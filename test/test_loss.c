...
int main() {
    ...
    CHECK(fabsf(mse_result - 0.01f) < TOLERANCE);

    CHECK(fabsf(calculate_cost(output_vecs, expected_vecs, BATCH_SIZE, mean_squared_error_loss.forward) - 0.012f) < TOLERANCE);
    ...
    for (size_t i = 0; i < dLoss_dActivated->length; i ++) {
        ...
        CHECK(fabsf(expected_mse_grads_data[i] - dLoss_dActivated->data[i]) <= TOLERANCE);
