...
void test_model_forward_backward() {
    ...
    CHECK(model->current_grads_accumulated == 1);

    model2d_average_grads(model);

    for (size_t i = 0; i < model->num_layers; ++i) {
        CHECK(model->averaged_gradients[i] != NULL);
        CHECK(model->averaged_gradients[i]->d_weights != NULL);
        CHECK(model->averaged_gradients[i]->d_biases != NULL);
    }
